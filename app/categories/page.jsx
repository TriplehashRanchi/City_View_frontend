...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    categoriesApi.list().then((response) => setCategories(unwrapListResponse(response))).catch(() => setCategories([]));
  }, []);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <Panel
        ...
        aside={<TextInput value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search categories" className="min-w-[280px]" />}
      >
        <DataTable
          columns={[
            { key: "name", label: "Name" },
            ...
            { key: "actions", label: "Edit", render: (row) => <Link href={`/categories/${row.id}/edit`} className="text-[#7b6540]">Open</Link> },
          ]}
          rows={rows}
